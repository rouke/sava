...
do
  ...
  echo 'CMD ["java", "-jar", "/'$SERVICE'.jar"]' >> $DIR/target/$SERVICE/Dockerfile
...
echo 'CMD ["java", "-Dfrontend.backend1.host=$BACKEND_1_HOST", "-Dfrontend.backend1.port=$BACKEND_1_PORT", "-Dfrontend.backend2.host=$BACKEND_2_HOST", "-Dfrontend.backend2.port=$BACKEND_2_PORT", "-jar", "/'$SERVICE'.jar"]' >> $DIR/target/$SERVICE/Dockerfile
docker build -t magneticio/sava-$SERVICE:$VERSION $DIR/target/$SERVICE
docker push magneticio/sava-$SERVICE:$VERSION
...
echo "building docker image:" $SERVICE
sed -e 's/_SERVICE_/'${SERVICE}'/g' $DIR/Dockerfile > $DIR/target/$SERVICE/Dockerfile
echo 'CMD ["java", "-Dfrontend.backend.host=$BACKEND_HOST", "-Dfrontend.backend.port=$BACKEND_PORT", "-jar", "/'$SERVICE'.jar"]' >> $DIR/target/$SERVICE/Dockerfile
docker build -t magneticio/sava-$SERVICE:$VERSION $DIR/target/$SERVICE
docker push magneticio/sava-$SERVICE:$VERSION
